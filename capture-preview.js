...
async function capture() {
  console.log("Capturing rendered quotation...");

  // First log in via fetch to get the session cookie
  ...
  // If mustChangePassword is true, change it quickly
  ...
  const url = `http://localhost:3000/quotations/${q.id}/print`;
  await page.goto(url, { waitUntil: "networkidle0" });

  const outputPath = path.join(__dirname, "quotation-rendered-preview.png");
